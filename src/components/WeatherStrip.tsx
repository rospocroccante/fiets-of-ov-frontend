...
export function WeatherStrip({ lat, lon }: { lat: number; lon: number }) {
  ...
  return (
    <div className="mb-3 flex items-center gap-4 overflow-x-auto rounded-card border border-slate-100 bg-white/80 px-4 py-3 shadow-sm backdrop-blur dark:border-white/10 dark:bg-night-surface/80">
      <div className="flex shrink-0 items-center gap-2" title={now.label}>
        ...
        <div>
          <p className="text-lg font-semibold leading-tight dark:text-night-text">{forecast.current.tempC}&deg;</p>
          <p className="text-[11px] leading-tight text-gray-500 dark:text-night-subtle">{now.label}</p>
        </div>
      </div>
      <span className="h-8 w-px shrink-0 bg-slate-100 dark:bg-night-border" />
        ...
        {forecast.hours.map((h) => {
          ...
          return (
            <li key={h.time} className="flex shrink-0 flex-col items-center" title={look.label}>
              <span className="text-[10px] font-medium text-gray-400 dark:text-night-subtle">{h.time}</span>
              <Glyph icon={look.icon} className="my-0.5 text-[20px] text-ams-sky dark:text-emerald-300" />
              <span className="text-[11px] font-semibold text-gray-700 dark:text-night-text">{h.tempC}&deg;</span>
              {h.precipProb >= 20 && (
